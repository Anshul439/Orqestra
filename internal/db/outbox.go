...
// GetUnprocessedOutbox fetches all unprocessed outbox entries and locks them for
// the duration of the returned transaction.
func GetUnprocessedOutbox(ctx context.Context, pool *pgxpool.Pool) ([]OutboxEntry, pgx.Tx, error) {
	tx, err := pool.Begin(ctx)
	if err != nil {
		return nil, nil, err
	}
...
func MarkOutboxProcessed(ctx context.Context, q DBTX, outboxID int) error {
	_, err := q.Exec(ctx, `
		UPDATE job_outbox
...
func CleanupProcessedOutbox(ctx context.Context, pool *pgxpool.Pool, olderThan time.Duration) (int64, error) {
